...
if specs/ 目录不存在:
  ...
  建议：
  - 使用 /speckitdriver:run <需求描述> 启动研发流程，生成首个功能规范
  - 或手动创建 specs/ 目录结构
  """
  终止流程
...
if specs/ 下无 NNN-* 功能目录或所有目录中均无 spec.md:
  ...
  建议：
  - 使用 /speckitdriver:run <需求描述> 生成功能规范
...
**产品模板**: plugins/speckitdriver/templates/product-spec-template.md
...
```text
prompt_source[sync] = "plugins/speckitdriver/agents/sync.md"  // 始终使用内置版本
```
